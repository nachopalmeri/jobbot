...
try:
    from backup import BackupManager, format_backup_list
except ImportError:
    from job_bot.backup import BackupManager, format_backup_list
...
def run_scheduler(app: Application, loop: asyncio.AbstractEventLoop):
    ...
    También ejecuta backups automáticos según configuración.
    """
    ...
    # Configurar backup automático si está habilitado
    backup_manager = None
    ...
    try:
        ...
        logger.warning(f"⚠️ No se pudo inicializar el sistema de backup: {e}")
    
    time.sleep(3 * 60)  # Esperar 3 min antes del primer chequeo
    
    # Parsear hora de backup desde config
    backup_schedule = getattr(config, 'BACKUP_SCHEDULE', '02:00')
    try:
        backup_hour, backup_minute = map(int, backup_schedule.split(':'))
    except ValueError:
    ...
    while True:
        ...
        # Verificar si es hora de hacer backup (una vez por día)
        if (backup_manager and 
            current_time.hour == backup_hour and 
            current_time.minute == backup_minute and
            (last_backup_date is None or last_backup_date.date() != current_time.date())):
            
            try:
                logger.info("📦 Ejecutando backup automático programado...")
                backup_info = backup_manager.create_backup(manual=False)
                ...
                admin_id = getattr(config, 'BACKUP_ADMIN_USER_ID', 0)
                if admin_id > 0:
                    try:
                        size_mb = backup_info.size_bytes / (1024 * 1024)
                        size_str = f"{size_mb:.2f} MB" if size_mb >= 1 else f"{backup_info.size_bytes} bytes"
                        
                        asyncio.run_coroutine_threadsafe(
                            app.bot.send_message(
                                chat_id=admin_id,
                                text=f"📦 <b>Backup automático completado</b>\n\n"
                                     f"🆔 ID: <code>{backup_info.id}</code>\n"
                                     f"📦 Tamaño: {size_str}\n"
                                     f"🔐 Checksum: <code>{backup_info.checksum[:16]}...</code>",
                                parse_mode=ParseMode.HTML
                            ),
                            loop
                        )
                    except Exception as e:
                        logger.warning(f"No se pudo notificar al admin sobre el backup: {e}")
                ...
                if admin_id > 0:
                    try:
                        asyncio.run_coroutine_threadsafe(
                            app.bot.send_message(
                                chat_id=admin_id,
                                text=f"⚠️ <b>Error en backup automático</b>\n\n"
                                     f"Error: <code>{str(e)[:200]}</code>\n\n"
                                     f"Revisá los logs del sistema.",
                                parse_mode=ParseMode.HTML
                            ),
                            loop
                        )
                    except Exception as notify_error:
                        logger.warning(f"No se pudo notificar al admin sobre el error: {notify_error}")
...
# ============================================================
# /backup — Comando de administrador para gestionar backups
# ============================================================
async def backup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Comando /backup para administradores del bot.
    
    Uso:
    /backup — Listar backups disponibles
    /backup create — Crear backup manual
    /backup restore [ID] — Restaurar un backup específico
    /backup verify [ID] — Verificar integridad de un backup
    """
    user = update.effective_user
    
    # Verificar si el usuario es admin (config.BACKUP_ADMIN_USER_ID)
    admin_id = getattr(config, 'BACKUP_ADMIN_USER_ID', 0)
    if admin_id == 0 or user.id != admin_id:
        await update.message.reply_text(
            "🔒 Este comando es solo para administradores del sistema."
        )
        return
    
    # Inicializar manager de backup
    try:
        manager = BackupManager()
    except Exception as e:
        logger.error(f"Error inicializando BackupManager: {e}")
        await update.message.reply_text(
            "❌ Error al inicializar el sistema de backup."
        )
        return
    ...
    if len(args) == 0:
        ...
        try:
            ...
            if not backups:
                await update.message.reply_text(
                    ...
                    "Usá /backup create para crear uno."
                )
                return
            
            message = format_backup_list(backups[:10])  # Mostrar últimos 10
            if len(backups) > 10:
            ...
            await update.message.reply_text(f"❌ Error al listar backups: {e}")
    
    elif args[0] == 'create':
        # Crear backup manual
        status_msg = await update.message.reply_text("🔄 Creando backup, por favor esperá...")
        ...
        try:
            ...
            await status_msg.edit_text(
                ...
                f"📦 Tamaño: {size_str}\n"
                f"🔐 Checksum: <code>{backup_info.checksum[:16]}...</code>\n"
                f"📁 Archivo: <code>{backup_info.backup_file}</code>",
                parse_mode=ParseMode.HTML
            )
            
        except Exception as e:
            logger.error(f"Error creando backup: {e}")
            await status_msg.edit_text(f"❌ Error al crear backup: {e}")
    
    elif args[0] == 'restore':
        # Restaurar backup
        if len(args) < 2:
            await update.message.reply_text(
                "⚠️ Uso: /backup restore [ID]\n\n"
                "Ejemplo: /backup restore 20241201_143022_manual"
            )
            return
        
        backup_id = args[1]
        status_msg = await update.message.reply_text(
            f"🔄 Restaurando backup <code>{backup_id}</code>...",
            parse_mode=ParseMode.HTML
        )
        
        try:
            # Verificar primero
            is_valid, msg = manager.verify_backup(backup_id)
            if not is_valid:
                await status_msg.edit_text(f"❌ Backup corrupto: {msg}")
                return
            ...
            await status_msg.edit_text(
                f"✅ <b>Backup restaurado exitosamente</b>\n\n"
                f"🆔 ID: <code>{backup_id}</code>\n"
                f"📁 Ubicación: <code>{restored_path}</code>\n\n"
                f"⚠️ El bot debe reiniciarse para usar la base de datos restaurada.",
                parse_mode=ParseMode.HTML
            )
            
        except Exception as e:
            logger.error(f"Error restaurando backup: {e}")
            await status_msg.edit_text(f"❌ Error al restaurar backup: {e}")
    
    elif args[0] == 'verify':
        # Verificar backup
        if len(args) < 2:
            await update.message.reply_text(
                "⚠️ Uso: /backup verify [ID]\n\n"
                "Ejemplo: /backup verify 20241201_143022_manual"
            )
            return
        
        backup_id = args[1]
        status_msg = await update.message.reply_text(
            f"🔄 Verificando backup <code>{backup_id}</code>...",
            parse_mode=ParseMode.HTML
        )
        ...
        try:
            ...
            if is_valid:
                await status_msg.edit_text(
                    f"✅ <b>Backup verificado correctamente</b>\n\n"
                    f"🆔 ID: <code>{backup_id}</code>\n"
                    f"📊 Estado: {msg}",
                    parse_mode=ParseMode.HTML
                )
            ...
            await status_msg.edit_text(f"❌ Error al verificar backup: {e}")
    
    else:
        # Comando no reconocido
        await update.message.reply_text(
            "📦 <b>Comandos de Backup disponibles:</b>\n\n"
            "/backup — Listar backups\n"
            "/backup create — Crear backup manual\n"
            "/backup restore [ID] — Restaurar backup\n"
            "/backup verify [ID] — Verificar integridad\n\n"
            "⚠️ Estos comandos son solo para administradores.",
            parse_mode=ParseMode.HTML
        )
