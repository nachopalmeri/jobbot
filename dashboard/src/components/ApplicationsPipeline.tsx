    ...
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      {allowCreate ? (
        <form
          ...
          className="mb-6 grid gap-3 rounded-3xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-2"
        >
          <input
            value={form.company}
      ...
      {applications.length === 0 ? (
        ...
        <div className={`grid gap-4 ${compact ? "xl:grid-cols-2" : "xl:grid-cols-4"}`}>
          {grouped.map((column) => (
            <div key={column.status} className="rounded-[24px] border border-slate-200 bg-slate-50 p-4">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-500">{column.label}</p>
                  <p className="text-2xl font-semibold text-slate-900">{column.items.length}</p>
                </div>
                <span
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-sm text-slate-500">{application.company}</p>
                      </div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      ...
                      {application.url ? (
                        <Link
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 rounded-full border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 hover:border-slate-300 hover:text-slate-900"
                        >
                          Ver aviso
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {applications.map((application) => (
                <tr key={application.id}>
                  ...
                  <td className="px-4 py-3 text-slate-500">{application.notes || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
